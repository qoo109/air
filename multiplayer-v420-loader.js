(() => {
  ...
  const required = (source, from, to, label) => {
    if (!source.includes(from)) throw new Error(`v4.2.0 patch missing: ${label}`);
  ...
  async function boot() {
    if (quick) {
      quick.disabled = true;
      quick.textContent = '🌐 TURN/WebRTC 載入中…';
    }

    let source = await fetch(`multiplayer-v408.js?v=4.2.0-${Date.now()}`, {
      cache: 'no-store',
    ...
    source = required(
      ...
      "storageKey: 'bubble-island-auth-v420'",
      'auth storage',
    );
    ...
    source = required(
      ...
      `transport = 'p2p';\n      routeLabel = 'WEBRTC';\n      clearTimeout(connectTimeout);\n      connectTimeout = null;\n      setStatus(\`已與 \${opponentName} 建立 WebRTC，準備開局…\`);\n      if (queueOnline) queueOnline.textContent = '連線狀態：檢查 P2P／TURN 路由';\n      detectWebRtcRoute();\n      if (role === 'host') sendStartOffer();`,
      'route detection',
    );
    ...
    source = source
      .replaceAll('hello-v408', 'hello-v420')
      .replaceAll('sdp-offer-v408', 'sdp-offer-v420')
      .replaceAll('sdp-answer-v408', 'sdp-answer-v420')
      .replaceAll('ice-v408', 'ice-v420')
      .replaceAll('relay-ready-v408', 'relay-ready-v420')
      .replaceAll('relay-control-v408', 'relay-control-v420')
      .replaceAll('relay-game-v408', 'relay-game-v420')
      .replace("transport: () => 'realtime-fast'", "transport: () => transport === 'p2p' ? routeLabel.toLowerCase() : 'realtime-fallback'");

    Function(`${source}\n//# sourceURL=multiplayer-v420-runtime.js`)();

    if (quick) {
      quick.disabled = false;
      quick.textContent = '⚡ TURN／WebRTC 快速配對';
    }
  }

  boot().catch((error) => {
    console.error('Bubble multiplayer v4.2.0 failed to load', error);
    if (quick) {
      quick.disabled = true;
      quick.textContent = '⚠ TURN／WebRTC 載入失敗';
    }
    if (status) status.textContent = `TURN／WebRTC 引擎載入失敗：${error.message}`;
  });
})();
