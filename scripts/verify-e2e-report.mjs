...
const TARGET = { averagePx: 25, p95Px: 50, maxPx: 90 };
...
function interpolateHost(samples, targetAt) {
  ...
  if (!(span > 0 && span <= 160)) return null;
...
if (drift.samples <= 50) throw new Error(`Only ${drift.samples} active-play drift samples were usable.`);
if (drift.averagePx > TARGET.averagePx || drift.p95Px > TARGET.p95Px || drift.maxPx > TARGET.maxPx) {
  throw new Error(`Active-play drift exceeded 25/50/90: avg=${drift.averagePx}, p95=${drift.p95Px}, max=${drift.maxPx}.`);
}

const visualP95 = [report.visual?.host?.p95JumpPx, report.visual?.guest?.p95JumpPx].map(Number);
if (visualP95.some((value) => !Number.isFinite(value) || value > 45)) throw new Error(`Visual P95 jump failed: ${visualP95.join(' / ')}.`);
