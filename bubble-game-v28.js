(() => {
  ...
  const ui = {
    ...
    combo:$('combo-label'), bestComboHud:$('best-combo-hud'),
  ...
  const DIFFICULTY = {
    easy:   {maxSpeed:235,response:.068,predict:.15,error:42,attack:.04},
    normal: {maxSpeed:320,response:.100,predict:.38,error:24,attack:.10},
    hard:   {maxSpeed:410,response:.140,predict:.63,error:11,attack:.19},
    boss:   {maxSpeed:500,response:.180,predict:.84,error:4,attack:.28}
  };
  ...
  const state = {
    ...
    stuckTimer:0, controlHintTimer:4.2,
  ...
  function arenaBounds() {
    ...
    const goalWidth=Math.min(width*.47,270);
    return {
  ...
  function beep(type='hit') {
    ...
    try {
      ...
      const config={
        hit:[470,.05,'sine',.04], wall:[210,.03,'triangle',.02],
        goal:[720,.22,'sine',.06], start:[610,.07,'sine',.035],
        boost:[900,.14,'triangle',.048], win:[1040,.32,'sine',.065],
        reset:[360,.1,'sine',.03]
      }[type] || [470,.05,'sine',.04];
  ...
  function updateRecords() {
    ...
    if (ui.bestComboHud) ui.bestComboHud.textContent=`最佳 x${state.bestCombo}`;
  }
  ...
  function saveBestCombo() {
    if (state.combo <= state.bestCombo) return;
    state.bestCombo=state.combo;
    localStorage.setItem('bubble_best_combo',String(state.bestCombo));
    updateRecords();
    ui.bestComboHud?.classList.remove('record-pulse');
    void ui.bestComboHud?.offsetWidth;
    ui.bestComboHud?.classList.add('record-pulse');
  }
  ...
  function serve(direction=1,lock=.38) {
    const bounds=arenaBounds();
    const speed=clamp(Math.min(innerWidth,innerHeight)*.75,300,450);
  ...
  function burst(x,y,color,count=16,forceStars=false) {
    const adjusted=lowPower?Math.ceil(count*.62):count;
    for (let i=0;i<adjusted;i++) {
      const angle=Math.random()*Math.PI*2;
      const speed=55+Math.random()*240;
      effects.push({
        x,y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,
        life:.32+Math.random()*.45,max:.77,size:3+Math.random()*7,
        color,star:forceStars || Math.random()<.16,rotation:Math.random()*6
      });
    }
  }

  function showMessage(text,duration=.85) {
    state.message=text;
    state.messageTimer=duration;
  }

  function normalizeBallSpeed(minimum=285,maximum=1180) {
  ...
  function collidePaddle(object,isPlayer) {
    ...
    if (isPlayer) {
      ...
      if (state.combo>0 && state.combo%3===0) {
        ...
        burst(ball.x,ball.y,world.player,13);
        beep('hit');
        vibrate(11);
      }
    } else {
      burst(ball.x,ball.y,world.ai,13);
      beep('hit');
      vibrate(8);
    }

    const rallyAcceleration=Math.min(1.03,1.007+state.rally*.001);
    ball.vx*=rallyAcceleration;
    ball.vy*=rallyAcceleration;
    normalizeBallSpeed(290,1180);
    state.shake=Math.min(4.8,2.8+state.rally*.06);
    updateHud();
  }
  ...
  function scoreGoal(playerScored) {
    ...
    if (playerScored) {
      state.playerScore++;
      showMessage(state.combo>=3?`連擊 x${state.combo}`:'好球！',.85);
    } else {
      state.aiScore++;
      showMessage('守住下一球！',.85);
    }

    burst(innerWidth/2,playerScored?bounds.top+8:bounds.bottom-8,playerScored?world.player:world.ai,42,true);
    state.flash=.18;
    state.shake=6.5;
    state.rally=0;
    state.roundPause=.95;
  ...
  function updatePaddles(dt,bounds) {
    ...
    let aiTargetY=bounds.top+ai.r+24;
    if (ball.vy<0 && state.serveLock<=0 && state.roundPause<=0) {
      ...
      aiTargetY=bounds.top+ai.r+bounds.height*(.10+aiConfig.attack);
    } else if (Math.abs(ball.y-bounds.middle)<bounds.height*.15) {
      aiTargetX+=(ball.x-innerWidth/2)*aiConfig.attack*.3;
    }
  ...
  function simulateBall(dt,bounds) {
    ...
    for (let i=0;i<steps;i++) {
      ...
      const inGoal=ball.x>bounds.goalLeft+ball.r*.22 && ball.x<bounds.goalRight-ball.r*.22;
  ...
  function update(dt) {
    ...
    if (ball.trail.length>(lowPower?10:15)) ball.trail.pop();

    const ballSpeed=Math.hypot(ball.vx,ball.vy);
    if (state.serveLock<=0 && ballSpeed<155) state.stuckTimer+=dt;
    else state.stuckTimer=0;
    if (state.stuckTimer>1.1) {
      if (ballSpeed<1) {
        const direction=Math.random()<.5?-1:1;
        ball.vx=(Math.random()-.5)*170;
        ball.vy=direction*290;
      } else {
        normalizeBallSpeed(290,1180);
      }
      state.stuckTimer=0;
      showMessage('珍珠重新加速！',.65);
      beep('reset');
    }
  ...
  function drawTrail() {
    ball.trail.forEach((point,index)=>{
      ctx.save();
      ctx.globalAlpha=(ball.trail.length-index)/ball.trail.length*.13;
