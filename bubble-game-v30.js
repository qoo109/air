(() => {
  ...
  const ui = {
    ...
    pauseBtn:$('pause-btn'), menu:$('menu'), pause:$('pause'),
    gameOver:$('game-over'), countdown:$('countdown'), countdownText:$('countdown-text'),
    ...
    totalWins:$('best-win'), bestStreak:$('best-streak'), currentStreak:$('current-streak'), bestCombo:$('best-combo')
  };
  ...
  const DIFFICULTY = {
    easy:   {maxSpeed:230,response:.066,predict:.14,error:44,attack:.035},
    normal: {maxSpeed:315,response:.098,predict:.37,error:25,attack:.095},
    hard:   {maxSpeed:405,response:.137,predict:.62,error:12,attack:.18},
    boss:   {maxSpeed:495,response:.177,predict:.83,error:5,attack:.27}
  };
  ...
  function stats(){
    return window.BubbleRanking?.getStats(playerName) || {
      wins:0,currentStreak:0,bestStreak:0,bestCombo:0
    };
  }
  ...
  function serve(direction=1,lock=.38){
    const bounds=arenaBounds();
    const speed=clamp(Math.min(innerWidth,innerHeight)*.73,290,430);
    const angle=Math.random()*.52-.26;
    ball.x=innerWidth/2;
    ball.y=bounds.middle;
  ...
  function pulseCombo(){
    if(!ui.combo) return;
    ui.combo.classList.remove('combo-pulse');
    void ui.combo.offsetWidth;
    ui.combo.classList.add('combo-pulse');
  }
  ...
  function burst(x,y,color,count=16,forceStars=false){
    ...
    const adjusted=lowPower?Math.ceil(count*.55):count;
    for(let i=0;i<adjusted;i++){
      const angle=Math.random()*Math.PI*2;
      const speed=50+Math.random()*220;
      effects.push({
        x,y,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,
        life:.3+Math.random()*.4,max:.7,size:3+Math.random()*7,
        color,star:forceStars||Math.random()<.14,rotation:Math.random()*6
      });
    }
  }

  function showMessage(text,duration=.78){
    state.message=text;
    state.messageTimer=duration;
  }

  function normalizeBallSpeed(minimum=278,maximum=1140){
    const speed=Math.hypot(ball.vx,ball.vy);
    if(!speed) return;
    if(speed<minimum){
  ...
  function collidePaddle(object,isPlayer){
    ...
    if(relative>=-15) return;

    ball.vx-=1.86*relative*nx;
    ball.vy-=1.86*relative*ny;
    ball.vx+=object.vx*.31;
    ball.vy+=object.vy*.31;
    object.hitCooldown=.072;
    state.rally++;
    ...
    if(isPlayer){
      ...
      if(state.combo%3===0){
        ball.vx*=1.065;
        ball.vy*=1.065;
        burst(ball.x,ball.y,world.accent,20,true);
        beep('boost');
        vibrate([12,12,22]);
        pulseCombo();
      }else{
        burst(ball.x,ball.y,world.player,11);
        beep('hit');
        vibrate(9);
      }
    }else{
      burst(ball.x,ball.y,world.ai,11);
      beep('hit');
      vibrate(6);
    }

    const rallyAcceleration=Math.min(1.026,1.006+state.rally*.0009);
    ball.vx*=rallyAcceleration;
    ball.vy*=rallyAcceleration;
    normalizeBallSpeed(282,1140);
    state.shake=reducedMotion?0:Math.min(4.2,2.4+state.rally*.05);
    updateHud();
  }
  ...
  function scoreGoal(playerScored){
    ...
    if(playerScored){
      state.playerScore++;
      showMessage(state.combo>=3?`連擊 x${state.combo}`:'好球！',.8);
    }else{
      state.aiScore++;
      state.combo=0;
      state.comboTimer=0;
    ...
    burst(innerWidth/2,playerScored?bounds.top+8:bounds.bottom-8,playerScored?world.player:world.ai,38,true);
    state.flash=reducedMotion?0:.15;
    state.shake=reducedMotion?0:5.5;
    state.rally=0;
    state.roundPause=.9;
  ...
  function updatePaddles(dt,bounds){
    ...
    const fingerLift=coarsePointer?clamp(player.r*1.03,40,59):0;
    ...
    const targetY=pointer.down
      ...
      :player.y;
    movePaddle(player,targetX,targetY,pointer.down?.54:.14,1640,dt);
  ...
  function updateEffects(dt){
    for(const effect of effects){
      ...
      effect.vx*=.94;
      effect.vy*=.94;
      if(!effect.star) effect.vy-=13*dt;
      effect.life-=dt;
  ...
  function simulateBall(dt,bounds){
    const speed=Math.hypot(ball.vx,ball.vy);
    const steps=clamp(Math.ceil(speed*dt/Math.max(6.5,ball.r*.52)),1,9);
    const step=dt/steps;
    ...
    for(let i=0;i<steps;i++){
      ...
      const inGoal=ball.x>bounds.goalLeft+ball.r*.3 && ball.x<bounds.goalRight-ball.r*.3;
  ...
  function update(dt){
    ...
    if(ball.trail.length>(lowPower||reducedMotion?8:14)) ball.trail.pop();
    ...
    if(state.stuckTimer>1.05){
      if(ballSpeed<1){
        ...
        ball.vx=(Math.random()-.5)*165;
        ball.vy=direction*285;
      }else{
        normalizeBallSpeed(285,1140);
      }
      state.stuckTimer=0;
      showMessage('重新發力！',.6);
      beep('reset');
  ...
  function drawBackground(){
    ...
    const count=lowPower||reducedMotion?5:9;
    ...
    for(let i=0;i<count;i++){
      ...
      if(world.kind==='ocean') drawFish(x,y+Math.sin(elapsed+i)*5,scale,['#399bd6','#f5a23a','#4fbe8b'][i%3],i%2?1:-1,.26);
      else if(world.kind==='night') drawStar((i*127)%innerWidth,(i*79)%innerHeight,3+scale*3,.5);
      else if(world.kind==='desert') i%2?drawCloud((i*101)%innerWidth,y,scale*.7,.26):drawCactus((i*111)%innerWidth,y,scale*.7);
      else drawCloud((i*103)%innerWidth,y,scale*.7,.3);
    }
  }
  ...
  function drawControlHint(){
    ...
    const lift=coarsePointer?clamp(player.r*1.03,40,59):0;
    ctx.save();
  ...
  async function countdown(){
    ...
    for(const value of ['3','2','1','GO']){
      ...
      beep('start');
      vibrate(value==='GO'?[14,14,22]:9);
      await new Promise(resolve=>setTimeout(resolve,value==='GO'?270:430));
    }
  ...
  function resumeGame(){
    ...
    state.mode='playing';
    showMessage('繼續！',.42);
    state.serveLock=Math.max(state.serveLock,.32);
    lastTime=performance.now();
    frame=requestAnimationFrame(loop);
  }
  ...
  function endGame(){
    ...
    if(ui.finalScore){
      ui.finalScore.textContent=
        `最終比分：${playerName} ${state.playerScore}：${state.aiScore} AI｜本場最高連擊 x${state.matchBestCombo}｜最長來回 ${state.bestRally}`;
    }
    if(won){
      beep('win');
      vibrate([32,24,62]);
  ...
  addEventListener('bubble-player-updated',()=>{readPlayer();updateHud();window.BubbleRanking?.sync?.();});
