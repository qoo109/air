(() => {
  ...
  function createGuest(name){
    return {
      id:crypto.randomUUID?.() || `guest-${Date.now()}-${Math.floor(Math.random()*10000)}`,
      name,
      avatar:'🐢',
      provider:'guest'
    };
  ...
  quickMatchBtn?.addEventListener('click',()=>{
    ...
    if(matchStatus) matchStatus.textContent='快速配對介面已就緒，等待真人連線伺服器上線…';
    clearInterval(queueTimer);
  ...
  function boot(){
    ...
    if(saved){
      try{
        ...
        if(user?.name){
          user.id ||= crypto.randomUUID?.() || `guest-${Date.now()}`;
          user.avatar='🐢';
          showMenu(user);
          return;
        }
