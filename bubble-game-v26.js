(() => {
  ...
  const WORLDS = {
    ocean:  { top:'#82d8f4', bot:'#e5fbff', rink:'#ccefe9', rink2:'#8fd7ca', edge:'#3c6175', line:'#fff',    player:'#61d0b0', ai:'#ff8e78', puck:'#fff4c7', accent:'#ffd05c', kind:'ocean',  drag:.99815 },
    desert: { top:'#9edff5', bot:'#ffe3a7', rink:'#ffd99a', rink2:'#efb267', edge:'#725268', line:'#fff6dc', player:'#63c8aa', ai:'#ed806f', puck:'#fff2c1', accent:'#a685e4', kind:'desert', drag:.99785 },
    village:{ top:'#a9e5f8', bot:'#effcff', rink:'#dcedb6', rink2:'#99cf79', edge:'#50697a', line:'#fff',    player:'#66c9ad', ai:'#f28c82', puck:'#fff2c0', accent:'#efa0ca', kind:'village',drag:.99825 },
    night:  { top:'#263b72', bot:'#6b579a', rink:'#566ba2', rink2:'#3f5282', edge:'#e7ecff', line:'#d8efff', player:'#6bd4c0', ai:'#ff8ca3', puck:'#fff0a5', accent:'#ffd55a', kind:'night',  drag:.99805 }
  };
  ...
  let playerName = 'YOU';
  let coarsePointer = matchMedia?.('(pointer: coarse)')?.matches ?? false;
  ...
  const state = {
    ...
    best:Number(localStorage.getItem('nah_best_win') || 0),
  ...
  function beep(type='hit') {
    ...
    try {
      ...
      const config = {
        ...
        start:[610,.075,'sine',.038], boost:[900,.15,'triangle',.052], win:[1040,.34,'sine',.07], reset:[360,.11,'sine',.032]
      }[type] || [470,.05,'sine',.04];
  ...
  function burst(x, y, color, count=16, forceStars=false) {
    for (let i=0; i<count; i++) {
      ...
      effects.push({
        ...
        color,
        star:forceStars || Math.random()<.18,
  ...
  function collidePaddle(object, isPlayer) {
    ...
    const distance = Math.hypot(dx, dy);
    const minimumDistance = ball.r + object.r*.82;
    ...
    const restitution = 1.92;
    ball.vx -= restitution*relative*nx;
    ball.vy -= restitution*relative*ny;
  ...
  function drawShell(xPos,yPos,color,scale=1) {
    ctx.save();
    ctx.translate(xPos,yPos);
  ...
  function drawArena() {
    ...
    drawShell(innerWidth/2,bounds.middle,world.accent);
  ...
  function drawFace(radius, angry=false) {
    ctx.fillStyle='#34495e';
    ctx.beginPath();
    ctx.arc(-radius*.22,-radius*.08,radius*.075,0,Math.PI*2);
    ctx.arc(radius*.22,-radius*.08,radius*.075,0,Math.PI*2);
    ctx.fill();
    ctx.strokeStyle='#34495e';
    ctx.lineWidth=2.5;
    ctx.beginPath();
    if (angry) ctx.arc(0,radius*.28,radius*.17,Math.PI*1.1,Math.PI*1.9);
    else ctx.arc(0,radius*.12,radius*.18,.1,Math.PI-.1);
    ctx.stroke();
  }

  function drawOctopus(object,color) {
    const radius=object.r;
    ctx.save();
    ...
    for (let i=-2;i<=2;i++) {
      ctx.beginPath();
      ctx.moveTo(i*radius*.24,radius*.34);
      ctx.quadraticCurveTo(i*radius*.34+Math.sin(elapsed*4+i)*5,radius*.9,i*radius*.42,radius*.8);
      ctx.stroke();
    }
    ctx.fillStyle=color;
    ctx.strokeStyle=world.edge;
    ctx.lineWidth=4;
    ctx.beginPath(); ctx.ellipse(0,-radius*.06,radius*.82,radius*.72,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
    ctx.fillStyle='rgba(255,255,255,.42)';
    ctx.beginPath(); ctx.ellipse(-radius*.23,-radius*.35,radius*.24,radius*.13,-.35,0,Math.PI*2); ctx.fill();
    drawFace(radius);
    ctx.restore();
  }

  function drawPufferfish(object,color) {
    const radius=object.r;
    ctx.save();
    ctx.translate(object.x,object.y);
    ctx.fillStyle=color;
    ...
    for (let i=0;i<12;i++) {
      ...
      const xPos=Math.cos(angle)*radius*.72;
      const yPos=Math.sin(angle)*radius*.72;
      ctx.beginPath();
      ctx.moveTo(xPos-Math.sin(angle)*4,yPos+Math.cos(angle)*4);
      ctx.lineTo(Math.cos(angle)*radius*1.02,Math.sin(angle)*radius*1.02);
      ctx.lineTo(xPos+Math.sin(angle)*4,yPos-Math.cos(angle)*4);
      ctx.closePath(); ctx.fill(); ctx.stroke();
    }
    ctx.beginPath(); ctx.arc(0,0,radius*.78,0,Math.PI*2); ctx.fill(); ctx.stroke();
    ctx.fillStyle='rgba(255,255,255,.38)';
    ctx.beginPath(); ctx.ellipse(-radius*.22,-radius*.28,radius*.22,radius*.12,-.35,0,Math.PI*2); ctx.fill();
    drawFace(radius,true);
    ctx.restore();
  }

  function drawPuck() {
    const radius=ball.r;
    const gradient=ctx.createRadialGradient(ball.x-radius*.35,ball.y-radius*.4,1,ball.x,ball.y,radius);
    ...
    ctx.strokeStyle=world.edge;
    ctx.lineWidth=3;
    ctx.beginPath(); ctx.arc(ball.x,ball.y,radius,0,Math.PI*2); ctx.fill(); ctx.stroke();
    ctx.fillStyle='rgba(255,255,255,.75)';
    ctx.beginPath(); ctx.arc(ball.x-radius*.35,ball.y-radius*.38,radius*.24,0,Math.PI*2); ctx.fill();
  }
  ...
  function drawControlHint() {
    ...
    ctx.textAlign='center';
    ctx.fillText(coarsePointer?'手指放在角色下方拖曳':'拖曳控制',player.x,player.y-player.r-18);
    ctx.restore();
  }
  ...
  function draw() {
    ...
    drawEffects();
    drawPuck();
    drawPufferfish(ai,world.ai);
    drawOctopus(player,world.player);
    drawControlHint();
    drawMessage();
    if (state.flash > 0) {
  ...
  async function startGame() {
    ...
    draw();
    await countdown();
    if (state.mode !== 'countdown') { state.starting=false; return; }
    state.mode='playing';
  ...
  function endGame() {
    ...
    if (ui.finalScore) ui.finalScore.textContent=`最終比分：${playerName} ${state.playerScore}：${state.aiScore} AI`;
    if (won) {
      state.best++;
      localStorage.setItem('nah_best_win',String(state.best));
      if (ui.best) ui.best.textContent=state.best;
      beep('win');
      vibrate([40,30,80]);
    }
  ...
  addEventListener('resize',()=>{
    coarsePointer=matchMedia?.('(pointer: coarse)')?.matches ?? coarsePointer;
    resize();
  });
